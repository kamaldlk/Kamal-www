...
angular.module("hexode.controllers")
    .controller('createApartmentCtrl', ["$scope", "$state", "api", "aptFacilities", "CONSTANTS", "$stateParams", "$cordovaToast",
        function ($scope, $state, api, aptFacilities, CONSTANTS, $stateParams, $cordovaToast) {
            ...
            if (angular.isDefined($stateParams.aptId)) {
                $scope.updateMode = $stateParams.mode ? true : false;
            ...
            $scope.addNewApartment = function (apartment) {
                if (!validateAppartForm(apartment)) return;
                apartment.user = {
                    id: $scope.user.id
                }
                apartment.knowuserid = $scope.user.id;

                apartment.latitude = 18.7780311;
                apartment.longitude = 99.0349443;

                api.Appartment.add(apartment, function (err, data) {
                    if (err) {
                        showMessage(CONSTANTS.ERROR.APT_CREATE_FAILED);
                    } else {
                        showMessage(CONSTANTS.SUCCESS.APT_CREATED);
                    }
                    $state.go("owner.home");
                });
            }
